...
def getComments():
    ...
    try:
        with dict_cur as cursor:
            sql = '''SELECT comments.*,users.username,COUNT(likes.postid) AS like_count,COUNT(commentreplies.replyid) as totalreplies,exists(select 1 from likes where likes.postid = comments.commentid and likes.userid = %s limit 1) as liked FROM comments 
